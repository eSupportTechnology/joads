...
use App\Models\JobAcademicEducation;
...
class EducationController extends Controller
{
    ...
    public function showAcademicEducation()
    {
        if (!auth()->check()) {
            return redirect()->route('login');
        }
        ...
        $user_id = auth()->id();
        $educations = JobAcademicEducation::where('job_seeker_id', $user_id)->get();
        $contacts = ContactUs::all();

        return view('User.jobseekerprofile.education-academic', compact('educations', 'contacts'));
    }
    ...
    public function academicUpdate(Request $request, $id)
    {
        ...
        try {
            $education = JobAcademicEducation::where('id', $id)
                ->where('job_seeker_id', auth()->id())
                ->firstOrFail();

            $education->update($validatedData);

            return redirect()->back()->with('success', 'Academic details updated successfully!');
        } catch (\Exception $e) {
            Log::error('Error in education update:', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
